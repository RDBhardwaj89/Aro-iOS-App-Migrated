...
// 5. Purpose: Robust movement engine with wake-up logic, C# snap-X physics, and petOpacity sync. Cleaned syntax to fix compiler errors.
...
class PetViewModel: ObservableObject, Identifiable {
    ...
    func update() {
        ...
        if !isChasing {
            ...
            let moveSpeed = (currentAnim?.baseSpeedX ?? 3.0) * speeds[model.speedId]
            x += (moveSpeed * directionX)
            
            let halfW = (45 * scale) / 2
            if x > (roomWidth - halfW) {
                directionX = -1
                x -= 2
            }
            if x < halfW {
    ...
    func wakeUpAndChase(targetX: CGFloat, targetY: CGFloat) {
        ...
        isChasing = true
        if dist > 200 {
            if currentAnimName != "Run" && currentAnimName != "Fly" {
                let chaseAnim = model.speciesId == 2 ? "Fly" : "Run"
                setupAnimation(chaseAnim)
            }
        } else {
        ...
        let speeds: [Double] = [0, 0.2, 0.5, 0.8, 1.0, 1.5, 2.0, 3.0]
        let speed = (dist > 200 ? 10.0 : 5.0) * speeds[model.speedId]
    ...
    func setupAnimation(_ name: String) {
        ...
        let minL = anim?.minLoops ?? 10
        let maxL = anim?.maxLoops ?? 20
        self.loopsRemaining = Int.random(in: minL...maxL)
    }
    ...
    func eatFood(completion: @escaping () -> Void) {
        isEating = true
        isChasing = false
        let eatAnim = model.speciesId == 2 ? "Eat" : "Sit"
        setupAnimation(eatAnim)
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            self.hunger = min(100, self.hunger + 35)
            self.isEating = false
            self.setupAnimation("Walk")
            self.saveToDatabase()
            completion()
        }
    }
