...
// 5. Purpose: Professional playground view with direct petOpacity access and integrated sprite renderer. Clean syntax.
...
struct ContentView: View {
            ...
            ZStack {
                ...
                ForEach(savedInstances) { instance in
                    if let brain = getBrain(for: instance, size: geo.size) {
                        ...
                        PetSpriteView(pet: brain, species: species)
                            .onTapGesture { selectedPetVM = brain }
                            .onAppear { startChaseLoop(brain) }
                    }
                }
                ...
                VStack {
                    ...
                    Capsule()
                        ...
                        .padding()
                        .onTapGesture { showingManager = true }
                }
            }
            .onAppear { SetupManager.seed(context: modelContext) }
            .sheet(isPresented: $showingManager) {
                ManagerView(activeFood: $activeFood, roomSize: geo.size)
            }
            ...
            if let closest = validFood.min(by: { abs($0.x - brain.x) < abs($1.x - brain.x) }) {
                ...
                if abs(closest.x - brain.x) < 30 && abs(closest.y - brain.y) < 30 {
                    if let idx = activeFood.firstIndex(where: { $0.id == closest.id }) {
                        activeFood[idx].isClaimed = true
                        brain.eatFood { activeFood.removeAll(where: { $0.id == closest.id }) }
                    }
                }
            } else {
                brain.stopChase()
    ...
    func getBrain(for instance: PetModel, size: CGSize) -> PetViewModel? {
        ...
        newBrain.roomWidth = size.width
        newBrain.roomHeight = size.height
...
struct PetSpriteView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ProgressView(value: pet.hunger, total: 100)
                .progressViewStyle(.linear)
                .frame(width: 40)
                .tint(pet.hunger < 20 ? .red : .green)
                .offset(y: -50)
            ...
            if let path = Bundle.main.path(forResource: pet.getSpritePath(species: species), ofType: "png"), let img = UIImage(contentsOfFile: path) {
                Image(uiImage: img).resizable().scaledToFit()
                    .shadow(color: pet.moodColor.opacity(0.8), radius: 10 * pulse)
                    .frame(width: 45 * pet.scale, height: 45 * pet.scale)
                    .scaleEffect(x: pet.directionX, y: 1)
                    .opacity(pet.petOpacity)
                    ...
                    .onAppear {
                        withAnimation(.easeInOut(duration: pet.hunger < 50 ? 0.5 : 1.2).repeatForever()) { pulse = 1.5 }
                    }
            }
        }
    }
