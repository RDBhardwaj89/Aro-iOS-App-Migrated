...
// 2. Version: 51.0
// 3. Date and time: Jan 9, 2026, 09:45 PM (IST)
// 4. Target for this file: Aro-CLI
// 5. Purpose: Multi-pet rendering stage with integrated chase logic and Ghost Pill navigation.
        ...
        ZStack(alignment: .top) {
            ...
            if let path = Bundle.main.path(forResource: pet.getSpritePath(species: species), ofType: "png"), let img = UIImage(contentsOfFile: path) {
                Image(uiImage: img).resizable().scaledToFit()
                    ...
                    .scaleEffect(x: pet.directionX, y: 1).opacity(pet.petOpacity).position(x: pet.x, y: pet.y)
                    ...
                    .onAppear { withAnimation(.easeInOut(duration: pet.hunger < 50 ? 0.5 : 1.2).repeatForever()) { pulse = 1.5 } }
            }
        }
    }
}
